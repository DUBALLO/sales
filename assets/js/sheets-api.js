// 월별매출 현황 JavaScript (CSV 직접 연결 버전)
...
async function loadSalesData() {
    try {
        ...
        salesData = rawData.map(item => ({
            date: new Date(item['주문일자'] || item.date || '2025-01-01'),
            type: item['구분'] || item.type || '사급매출',
            contractName: item['계약명'] || item.contractName || '계약명 없음',
            customer: item['거래처'] || item.customer || '거래처 없음',
...
function loadSampleDataFallback() {
    ...
    generateReport();
    CommonUtils.showAlert('샘플 데이터를 표시합니다. 실제 데이터 연결을 확인해주세요.', 'warning');
}
